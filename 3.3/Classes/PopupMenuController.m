...
//	self.view.frame = CGRectMake( 0, 0, 95, 44);
	self.view.frame = CGRectMake( 0, 0, 65, 44);
	
	NSArray* segments = [[NSArray alloc] initWithObjects:@"", nil];
	segmentControl_ = [[UISegmentedControl alloc] initWithItems:segments];
	segmentControl_.segmentedControlStyle = UISegmentedControlStyleBar;
	[segments release];
	segmentControl_.userInteractionEnabled = NO;
	segmentControl_.frame = CGRectMake( -7 , 7, 77, 30 );
	[self.view addSubview:segmentControl_];
	[segmentControl_ release];
	
	mainLabel_ = [[UILabel alloc] initWithFrame:CGRectMake( 0, 0, self.view.frame.size.width, 42)];
	mainLabel_.backgroundColor = [UIColor clearColor];
	mainLabel_.textAlignment = UITextAlignmentCenter;
	mainLabel_.font = [UIFont boldSystemFontOfSize:12.0f];
	mainLabel_.textColor = [UIColor whiteColor];
	mainLabel_.shadowColor = [UIColor blackColor];
	[self.view addSubview:mainLabel_];
	[mainLabel_ release];
	
	mainView_ = [[UIView alloc] initWithFrame:CGRectZero];
...
- (void)rebuildPopupMenu:(int)res {
	...
	PopupMenuCell *cell = [[PopupMenuCell alloc] initWithFrame:CGRectMake( 0, height, CELL_WIDTH, CELL_HEIGHT_TIP) mode:0];
	
	height += CELL_HEIGHT_TIP;
	cell.number = [cells_ count];
	cell.label.text = NSLocalizedString( @"NewComming", nil );
	cell.from = -1;//res - 200 < 0 ? 1 : res - 200;
	cell.to = -1;//res;
...
/*	
	...
	[cell release];
*/	
	int numberOfCell = res == 1001 ? res / 200 : res / 200 + 1;
	
	mainView_.frame = CGRectMake( ( self.view.frame.size.width - CELL_WIDTH ) / 2, 0, CELL_WIDTH+1, CELL_HEIGHT * ( numberOfCell - 1 ) + CELL_HEIGHT_TIP * 2 );
	...
	for( i = 0; i < numberOfCell; i++ ) {
		...
		if( i != numberOfCell - 1 ) {
			...
			cell.label.text = [NSString stringWithFormat:@"%d - %d", i*200 +1, res == 1001 ? 1000 : res];
			cell.from = i * 200 + 1;
			cell.to = res;
		}
		[cells_ addObject:cell];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	CGRect defaultRect = mainView_.frame;
	defaultRect.origin.y = -defaultRect.size.height + 10;
	mainView_.frame = defaultRect;
	
	for( id obj in cells_) {
		[mainView_ addSubview:obj];
	}
}
...
//	DNSLogMethod
	...
	UIView* view = [mainView_ hitTest:[touch locationInView:mainView_] withEvent:nil];
...
//	DNSLogMethod
	...
	UITouch *touch = [touches anyObject];
	PopupMenuCell* view = (PopupMenuCell*)[mainView_ hitTest:[touch locationInView:mainView_] withEvent:nil];
	
	[mainView_ removeFromSuperview];
	if( view ) {
		[(ThreadViewController*)delegate_ setFrom:view.from to:view.to];
		mainLabel_.text = view.label.text;
	}
}
