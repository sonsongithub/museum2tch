...
- (void)pushImageButton:(id)sender {
	...
	[UIAppDelegate openHUDOfString:NSLocalizedString( @"Downloading", nil )];
	...
	if( error ) {
		...
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString( @"ImageSaving", nil)
	...
	if( img == nil ) {
		[UIAppDelegate closeHUD];
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString( @"ImageSaving", nil)
														message:NSLocalizedString( @"FailedDecodingImage", nil)
													   delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
		[alert show];
		[alert release];
		return;
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
	...
	safariButton_.enabled = NO;
	[UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
	self.visibleViewController.title = NSLocalizedString( @"Error", nil );
}
