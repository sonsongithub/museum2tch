...
#define			NUMBER_OF_JAPANESE_CHAR_CODES	27
NSArray*		codes = nil;
NSMutableArray*	codeNames = nil;
NSDictionary*	specialTags = nil;
...
void convertHTMLTag(NSMutableString* str){
	...
	NSArray *keys = [specialTags allKeys];
	int i;
	for( i = 0; i < [keys count]; i++ ) {
		id before = [keys objectAtIndex:i];
		id after = [specialTags valueForKey:before];
...
void initDictionariesForHTMLDecode(){
	...
	if( specialTags == nil ) {
		specialTags = [[NSDictionary dictionaryWithObjectsAndKeys:
					@"\n",		@"<br>",
					nil
				] retain];
	}
...
void readyForDecodingTypeString() {
	if( codeNames == nil ) {
		codeNames = [[NSMutableArray array] retain];
...
// insert new line at specifiyed width and font size
NSString* divideStringWithWidthAndFontSizeOnlyLines( NSString* str, float width, float fontSize, int lines ) {
	...
	float lineWidth = 0;
	float alphabet_width = fontSize * RATIO_CHAR_J_AND_A;
	float japanese_width = fontSize;
	int	numberOfLines = 0;
	...
	while( strIndex < textLen) {
		...
		if( c >> 8 )	// check single byte or multi bytes
			lineWidth += japanese_width;
		else
			lineWidth += alphabet_width;
		if( lineWidth > width ) {
			lineWidth = 0;
			[newStr appendFormat: @"\n"];
			numberOfLines++;
			if( numberOfLines == 3 ) {
				if( [newStr length] > 3 )
					[newStr replaceCharactersInRange:NSMakeRange( [newStr length] - 3, 3 ) withString:@"..."];
				return newStr;
			}
		}
		strIndex++;
