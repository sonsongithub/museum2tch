...
- (void)pushAddButton:(id)sender {
	...
	if( self.threadDat.path != nil && self.threadDat.dat != 0 ) {
		UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:LocalStr( @"BookmarkThisThread?" ) 
														   delegate:self
...
- (void) didFinishLoading:(id)data response:(NSHTTPURLResponse*)response {
	...
	CFTimeInterval start_time, end_time;
	...
	if( [data length] != bytes && [data length] > 0) {
		...
		DatParser* p = [[DatParser alloc] init];
		[p parse:data appendToDat:candidateDat];
		end_time = CFAbsoluteTimeGetCurrent();
		DNSLog(@"DatParser %f seconds", end_time - start_time );
		
		start_time = CFAbsoluteTimeGetCurrent();
		//ThreadDatParser* parser = [[ThreadDatParser alloc] init];
		//[parser parse:data appendToDat:candidateDat];
		end_time = CFAbsoluteTimeGetCurrent();
		DNSLog(@"ThreadDatParser %f seconds", end_time - start_time );
